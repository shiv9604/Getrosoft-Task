...
import { Observable, catchError, concatMap, debounceTime, distinctUntilChanged, map, of, retry, switchMap } from "rxjs";
            ...
            return of(control.value).pipe(
                ...
                switchMap((value) => {
                    return this.userService.checkUserName(value).pipe(
                        ...
                        catchError((e) => {
                            this.toasterService.showError(e?.message || 'An error occurred while checking user name availability');
                            return of(null)
                        }))
                })
            )
